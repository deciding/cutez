...
def concat_to_interleave(a: cute.Tensor, dim: int) -> cute.Tensor:
    ...
    stride = (
        *a.stride[:dim],
        (half * a.stride[dim], a.stride[dim]),
        *a.stride[dim + 1 :],
    )
...
def permute_Cregs_b32_for_stsm(t: cute.Tensor) -> None:
    ...
    assert cute.size(t.shape) % 4 == 0, (
        "Tensor size must be a multiple of 4 for b32 permutation"
    )
    ...
    for i in cutlass.range(cute.size(t.shape) // 4, unroll_full=True):
        for r in cutlass.range(2, unroll_full=True):
            ...
            left0 = cute.arch.shuffle_sync(
                left0, offset=left_idx, mask_and_clamp=mask_and_clamp
            )
            # a b | f d | c e | h g -> a e | f b | c g | h d
            right0 = cute.arch.shuffle_sync(
                right0, offset=right_idx, mask_and_clamp=mask_and_clamp
            )
...
def tile_atom_to_shape_SF_strided(
    ...
    outer = cute.make_layout(
        (rmn, rk), stride=(sf_strides[1] // 512, sf_strides[2] // 512)
    )
    sf_layout = cute.blocked_product(atom, outer)
    if const_expr(len(shape) == 3):
        sf_layout = cute.append(
            sf_layout, cute.make_layout(shape[2], stride=sf_strides[0])
        )
    return sf_layout
